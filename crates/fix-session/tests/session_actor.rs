...
use fix_session::{
    ...
    spawn_initiator, spawn_initiator_with_dictionary,
};
use fix_store::{CommandPhase, MemoryStore, StoreWorker};
use std::sync::Arc;
...
async fn initiator_journals_and_sends_logon_using_the_recovered_sequence() {
    ...
    let logon = parse_frame(&frames[0], &ParseDictionary::new()).expect("parsed Logon");
...
async fn inbound_test_request_is_committed_then_answered_with_matching_heartbeat() {
    ...
    let heartbeat = parse_frame(&frames[0], &ParseDictionary::new()).expect("parsed Heartbeat");
...
async fn a_high_sequence_is_buffered_then_drained_after_a_gap_fill() {
    ...
    let resend = parse_frame(&resend[0], &ParseDictionary::new()).expect("parsed ResendRequest");
...
async fn application_submit_is_idempotent_and_replays_with_possdup_on_request() {
    ...
    let order = parse_frame(&order[0], &ParseDictionary::new()).expect("parsed order");
    ...
    let replay = parse_frame(&replay[0], &ParseDictionary::new()).expect("parsed replay");
...
async fn resend_of_admin_only_range_uses_sequence_reset_gap_fill() {
    ...
    let gap_fill = parse_frame(&gap_fill[0], &ParseDictionary::new()).expect("parsed GapFill");
...
async fn established_session_sends_heartbeat_after_outbound_inactivity() {
    ...
    let heartbeat = parse_frame(&heartbeat[0], &ParseDictionary::new()).expect("parsed Heartbeat");
...
async fn inbound_silence_sends_test_request_and_matching_heartbeat_clears_it() {
    ...
    let test_request = timeout(Duration::from_secs(4), async {
        ...
        loop {
            ...
            {
                let message =
                    parse_frame(&frame, &ParseDictionary::new()).expect("parsed session message");
...
async fn explicit_logout_transitions_to_logout_sent_and_accepts_peer_response() {
    ...
    let logout = parse_frame(&logout[0], &ParseDictionary::new()).expect("parsed Logout");
...
#[tokio::test]
async fn configured_dictionary_blocks_an_unknown_inbound_message_type() {
    let (client, mut server) = duplex(8192);
    let store = StoreWorker::spawn(MemoryStore::new(b"audit-test-key"));
    ...
    let handle = spawn_initiator_with_dictionary(
        client,
        SessionConfig {
            begin_string: "FIX.4.4".to_owned(),
            sender_comp_id: "CLIENT".to_owned(),
    ...
    let unknown = encode_message(
        b"FIX.4.4",
        &[
            Field::new(35, Bytes::from_static(b"X")),
            Field::new(49, Bytes::from_static(b"SERVER")),
            Field::new(56, Bytes::from_static(b"CLIENT")),
            Field::new(34, Bytes::from_static(b"2")),
            Field::new(52, Bytes::from_static(b"20260726-15:00:01.000")),
    ...
    assert!(error.contains("unknown MsgType X"), "{error}");
}
