...
- 所有 FIX 消息必须先通过 `fix-protocol` framing/codec；配置了字典的 daemon 不得绕过 `CompiledDictionary::validate`。
...
- 新依赖必须说明 feature、纯 Rust 状态、build.rs 和 transitive native 风险，并重新运行锁文件 denylist。
